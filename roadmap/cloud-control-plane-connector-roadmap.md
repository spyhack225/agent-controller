...
| Connector npm package | implemented, pack-tested, and protected-release automated | Clean install; exact package/version/tag/commit authorization; dependency-free tarball bounds; npm dry-run; protected OIDC/provenance workflow with immutable-version refusal, redacted evidence, and clean external exact-version verification design; native credential, T3 ownership, update/rollback, rotation, and platform-mocked service lifecycle | Commit the authoritative GitHub repository metadata, configure the npm trusted publisher and protected environment, execute publication; live macOS/Linux/Windows service/T3/native-store/sleep/update/rotation proof |
...
| CG-10 | **Repository scan, CI enforcement, adapter error redaction, and release guards are implemented; tracked-file remediation remains.** The path/signature gate blocks every non-example `controller_config*` variant, Convex adapter failures expose only local allowlisted metadata, and connector release refuses missing/`UNLICENSED` licensing. Both live CrowPanel variants were removed from the index and from history on 2026-09-08 before the first push to GitHub (a pre-rewrite mirror remains on the maintainer's machine until the rotation below is done); a previously exposed Convex gateway credential and the purged bench device secret still need external rotation. | Preserve needed local copies, remove both live variants from the index/history as appropriate, rotate every credential that may have appeared, and choose/add the connector's authoritative license and repository metadata. | `git ls-files` excludes live configs; examples contain placeholders only; local and CI secret scans pass; external rotation is recorded; package/license/repository release checks pass. Index/history changes and external rotation require maintainer authorization. |
...
1. **Release preconditions:** resolve CG-10's tracked-config/rotation boundary and repeat CG-11's
