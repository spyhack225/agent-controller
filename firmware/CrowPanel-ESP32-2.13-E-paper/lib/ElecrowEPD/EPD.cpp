...
void EPD_ShowPicture(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t pic[], uint8_t color)
{
  ...
  for (i = 0; i < TypefaceNum; i++)
  {
    ...
    /* PS：由于图片大小是248*122的，为保持显示和之前一致*/
    if (width < 250) {
        for(int row = 0; row < 122; row++) {
            EPD_DrawPoint(248, row, BLACK);
            EPD_DrawPoint(249, row, BLACK);
        }
    }
  }
}
